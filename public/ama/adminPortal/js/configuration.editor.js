angular.module('configuration.editor', ['ui.sortable'])
...
.directive('jsonconfig', ["$compile", function($compile) {
  return {
    restrict: 'E',
    scope: {
      child: '=',
    ...
    link: function(scope, element, attributes) {
        ...
        if (scope.collapsed) {
            scope.chevron = "glyphicon-triangle-right";
        } else {
            scope.chevron = "glyphicon-triangle-bottom";
        }
		...
		scope.duplicateKey = function(obj, key) {
			var newKey = "NewDuplicate";
			if (getType(obj) == "Object") {	
				alert("A new json node will be added as NewDuplicate. Please edit the name as per your requirement ");
				if (obj[newKey]) {					
                    if( !confirm('An item with the name "'+NewDuplicate
                ...
                console.error("object to duplicate from was " + obj);
            } 
        };
            ...
            if (getType(obj) == "Object") {
                ...
                if (scope.keyName == undefined || scope.keyName.length == 0){
                    ...
                    switch(scope.valueType) {
                        ...
                        case numberName: obj[scope.keyName] = scope.possibleNumber(scope.valueName);
                                         break;
                        case objectName:  obj[scope.keyName] = {"ItemName": "ItemValue"};
                                        break;
                        case arrayName:   obj[scope.keyName] = ["NewItem"];
                                        break;
                ...
                switch(scope.valueType) {
                    ...
                    case numberName: obj.push(scope.possibleNumber(scope.valueName));
                                     break;
                    case objectName:  obj.push({"ItemName": "ItemValue"});
                                    break;
                    case arrayName:   obj.push(["NewItem"]);
                                    break;
        ...
        var switchTemplate = 
            '<span ng-switch on="getType(val)" >'
                + '<jsonconfig ng-switch-when="Object" child="val" type="object" default-collapsed="defaultCollapsed"></jsonconfig>'
                + '<jsonconfig ng-switch-when="Array" child="val" type="array" default-collapsed="defaultCollapsed"></jsonconfig>'
        ...
        if (scope.type == "object"){
                ...
                + '<span class="block" ng-hide="key.indexOf(\'_\') == 0" ng-repeat="(key, val) in child">'
					...
					+'<li class="dropdown">'
					  ...
					  +'<ul class="dropdown-menu">'
						+'<li><a href="#" ng-click="duplicateKey(child, key)">Duplicate</a></li>'
						+'<li><a href="#" ng-click="$parent.showAddKey = true">Add</a></li>'
						+'<li><a href="#" ng-click="deleteKey(child, key)">Remove</a></li>'
					  +'</ul>'
					+'</li>'
				  +'</ul>'
                    // object key
            ...
            + '<div class="jsonContents" ng-hide="collapsed">'
                + '<div class="arrayOl" ui-sortable="sortableOptions" ng-model="child">'
                    // repeat
                  + '<ul ng-repeat="(key, val) in child track by $index">'
                  	+'<ul class="nav navbar-nav">'
						+'<li class="dropdown">'
						  +'<a href="#" class="dropdown-toggle" data-toggle="dropdown"><span class="glyphicon glyphicon-user pull-right"></span></a>'
						  +'<ul class="dropdown-menu">'
							+'<li><a href="#" ng-click="duplicateKey(child, key)">Duplicate</a></li>'
							+'<li><a href="#" ng-click="$parent.showAddKey = true">Add</a></li>'
							+'<li><a href="#" ng-click="deleteKey(child, $index)">Remove</a></li>'
						  +'</ul>'
						+'</li>'
				   +'</ul>'                       
