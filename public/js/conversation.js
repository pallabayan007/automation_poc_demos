...
//var enableVoice = null;
 var counter = 0;
 var isVoiceBasedConv = false;
...
// Set up the input box to underline text as it is typed
  ...
  function displayMessage(newPayload, typeValue) {
    ...
    if (newPayload.input){
		...
		if (Api.getResponsePayload().context.accessReqdAtCurrentLocation=='yes'){
			console.log("=====newPayload.input.TEST==== "+Api.getResponsePayload().context.accessReqdAtCurrentLocation);
			getLocation();
			Api.getResponsePayload().context["accessReqdAtCurrentLocation"] = null;
		}
	}
    if (newPayload.output){
    	console.log("=====newPayload.output==== "+newPayload.output.text);
		console.log("================isVoiceBasedConv value================="+isVoiceBasedConv);
		if(isVoiceBasedConv){
			navigator.mediaDevices.getUserMedia({ audio: true });
			textToSpeech(newPayload.output.text);
		}
	}
    ...
    console.log("*******inside displayMessage "+textExists);
  ...
  function inputKeyDown(event, inputBox) {
    ...
    if ((event.keyCode === 13 || event.key === 'Enter') && inputBox.value) {
