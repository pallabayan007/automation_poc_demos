...
//poll for response
  function parseResponse(data, timer, token, automationExecutionID,currentuser,operationdetailjson,count,input_context) {
				...
				if(automationStatusMsg=="In Progress"){
					if(count==1){
						console.log ( "\n====== Test-1:- ");
						SocketManager.emit(currentuser+":OtherResponse", 'I am still processing your request, please wait!');
					}
					else if(count==2){
						console.log ( "\n====== Test-2:- ");
						SocketManager.emit(currentuser+":OtherResponse", 'I am almost done, just a few minutes');
					}
						if (count>=max_count){
